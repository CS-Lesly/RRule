...
public sealed class RecurrenceRuleComponent : CompositeComponent
{
    public override Component Composition
        => COLON + Parameter + (SEMICOLON + Parameter).ZeroOrMore();
    ...
    {
        if (!tokens.TryGetSingleResultAfterLiteral<Frequency>([new LiteralComponent("FREQ"), EQUALS], out var frequency))
        {
            stream.Fail("Missing required FREQ parameter in recurrence rule");
            return null;
        }
